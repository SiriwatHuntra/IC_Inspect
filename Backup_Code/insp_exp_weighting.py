...
# ---- OCR Constants ----
OCR_CONF_EXPECTED  = 0.80   # expected char fast-path: return immediately if ≥ this
OCR_EARLY_EXIT_IOC = 0.70   # skip secondary group if primary best ≥ this
OCR_MIN_CONF       = 0.35   # below this → report "?" (unreadable)
...
class InspectionEngine:
    ...
    def ocr_identify(canvas:        np.ndarray,
        ...
        """
        OCR identification via canvas IoU against all saved templates.

        Three-stage search:

        Stage 1 — Expected char fast path
            Score the expected character's template directly.
            If IoU >= OCR_CONF_EXPECTED (0.80) return immediately —
            the normal case (correct mark, clean image) exits here.

        Stage 2 — Group search (fallback, below 0.80)
            Templates split into alpha (A-Z) and numeric/special groups.
            The group matching expected_char type is searched first.
            Expected char is excluded (already scored) but its score is
            seeded as the starting best so it remains a candidate.
            Early exit if best reaches OCR_EARLY_EXIT_IOC (0.70).

        Returns (char: str, conf: float).
        Returns ("?", conf) if nothing clears OCR_MIN_CONF (0.35).
        """
        ...
        if not expected_char:
            ...
            for name, tmpl in all_templates.items():
                ...
                if tmpl_canvas is None:
                    continue
                score = InspectionEngine._check_similarity(canvas, tmpl_canvas)
                if score > best_conf:
                    best_conf = score
                    best_char = name
        ...
        if exp_tmpl is not None:
            exp_canvas = exp_tmpl.get("canvas")
            if exp_canvas is not None:
                exp_conf = InspectionEngine._check_similarity(canvas, exp_canvas)
    ...
    def compare_roi(self,
        ...
        # Confident modification for retry
...
class InspectionController:
    ...
    def _step3_inspect_fonts(self, image_bgr, display,
        ...
        for slot_idx, letter in enumerate(grid_letters):
            ...
            # ── OCR — runs for every slot, expected or not ───────────
            gray_slot = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY) \
                        if roi.ndim == 3 else roi
            _, slot_canvas = self._eng._check_presence(gray_slot, mold_size)
            ...
            # ── Retry on recoverable failures (steps 3/4/5) ──────
            if res["defect_step"] in {3, 4, 5}:
                rx_w = int(roi_w * 1.15)
                rx_h = int(roi_h * 1.15)
